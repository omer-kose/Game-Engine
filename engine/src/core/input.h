...
typedef enum buttons 
{
    ...
    BUTTON_RIGHT,
    BUTTON_MIDDLE,
    BUTTON_MAX_BUTTONS // Mouses can have more than just 3 buttons. So, we keep this here in case of any expansions
} buttons;

/*
    The ‘##’ preprocessing operator performs token concatenation. When a macro is expanded, the two tokens on either side of each ‘##’ operator are combined into a 
    single token, which then replaces the ‘##’ and the two original tokens in the macro expansion.

    Basically, it concatenates two tokens. For example, if the name is BACKSPACE, KEY_##name will be KEY_BACKSPACE
*/
#define DEFINE_KEY(name, code) KEY_##name = code

/*
    Combination of ASCII codes and Window's keycodes.
*/
typedef enum keys 
{

...
{
    // Whether these queues are existent or not
    b8 graphics;
    b8 present;
    b8 compute;
    b8 transfer;
...
b8 vulkan_device_create(vulkan_context* context)
{
    ...
    KINFO("Logical device is created.");
...
void vulkan_device_destroy(vulkan_context* context)
{
    ...
    KINFO("Destroying the logical device");
...
)
{
    ...
    // We aim to have a queue family specifically does the transfer operations and not shared with Graphics and Compute.
    ...
    for(u32 i = 0; i < queue_family_count; ++i) 
    {
        ...
        if(queue_families[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) 
        {
            ...
            ++current_transfer_score; // Every Queue family with Graphics support already implicity supports transfer
        }
        ...
        if(queue_families[i].queueFlags & VK_QUEUE_COMPUTE_BIT) 
        {
            ...
            ++current_transfer_score; // Every Queue family with Compute support already implicity supports transfer
        }

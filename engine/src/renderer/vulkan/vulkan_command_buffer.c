...
{
    // To guarantee everything in the struct is zeroed out
    kzero_memory(out_command_buffer, sizeof(vulkan_command_buffer));

    VkCommandBufferAllocateInfo allocate_info = {VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO};
    allocate_info.commandPool = pool;
    // Secondary command buffers can be used withing another command buffer but they cannot be used on their own.
...
)
{
    ...
    if(is_simultaneous_use) // if the same command buffer will be used with the other queues
    {

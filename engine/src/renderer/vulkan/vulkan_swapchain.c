...
)
{
    ...
    // OUT_OF_DATE can be thrown out when window is resized for example. It means we need to recreate the swapchain as it is not valid anymore
...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain)
{
    ...
    // If the preferred is not available just take the first available
    if(!found)
    {
        swapchain->image_format = context->device.swapchain_support.formats[0];
    }

    // FIFO Present Mode guaranteed to exist per Vulkan Spec.
    ...
    // Requery swapchain support as a safety measurement against stuff like display and resolution change
    ...
    swapchain_create_info.imageUsage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT; // we are going to use these images to render to the color buffer

    // Setup the queue family indices
    if(context->device.graphics_queue_index != context->device.present_queue_index) 
    {
        // if graphics and present queues are seperate, then images in the swapchain are shared between them concurrently.
    ...
    {
        // if the two queues are the same, the images won't be shared.
    ...
    for(u32 i = 0; i < swapchain->image_count; ++i) 
    {
        ...
        view_info.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT; // how images will be used
    ...
    // We set the format of the swapchain images to be color attachments, so swapchain does not provide us with depth images.
    // We need to explicitly create our own depth images.
    ...
    vulkan_image_create(
        ...
        VK_IMAGE_TILING_OPTIMAL, // Let GPU to decide tiling
        VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT,
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, // Use device local memory (GPU memory not shared)
        true,
        VK_IMAGE_ASPECT_DEPTH_BIT,
        &swapchain->depth_attachment
    );

    KINFO("Swapchain is created successfully.");

}

void destroy(vulkan_context* context, vulkan_swapchain* swapchain)
{
    // Make sure that the image is not being used by the device

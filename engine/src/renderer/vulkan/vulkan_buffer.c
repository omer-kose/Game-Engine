...
)
{
    ...
    buffer_create_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE; // NOTE: Buffer only used in one queue
    ...
    // We need to explicitly allocate that.
    // Gather memory requirements
    VkMemoryRequirements requirements;
    vkGetBufferMemoryRequirements(context->device.logical_device, out_buffer->handle, &requirements);
    // Find a suitable memory type based on Vulkan's requirements on the buffer and our needs
    ...
    {
        KERROR("Unable to create the vulkan buffer because the required memory allocation failed. Error: %i", result);
        return false;
    }

    if(bind_on_create)
    {
        vulkan_buffer_bind(context, out_buffer, 0); // Bind from the beginning
    }
...
{
    // A buffer is immutable. So, we will recreate it.
    // Create new buffer.
    ...
    // Copy the previous content to the new buffer
    // To work on the new buffer, bind it.

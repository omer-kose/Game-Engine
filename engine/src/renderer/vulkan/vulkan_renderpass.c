...
(
    ...
    f32 x, f32 y, f32 w, f32 h, // Which area we will render to
    f32 r, f32 g, f32 b, f32 a, // Clear color
    f32 depth,
    u32 stencil
)
{
    // Copy the parameters into the renderpass
    out_renderpass->x = x;
    out_renderpass->y = y;
    out_renderpass->w = w;
    out_renderpass->h = h;
    ...
    color_attachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR; // What operation occurs while loading
    color_attachment.storeOp = VK_ATTACHMENT_STORE_OP_STORE;  // What operation occurs while storing
    ...
    // Connect color attachment and the reference in the subpass
    ...
    dependency.srcSubpass = VK_SUBPASS_EXTERNAL; // Main subpass is the first (and only for now). 
    dependency.dstSubpass = 0;
    dependency.srcStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
    dependency.srcAccessMask = 0;
    dependency.dstStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
    dependency.dstAccessMask = VK_ACCESS_COLOR_ATTACHMENT_READ_BIT | VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT; // We want to read and write to it.
...
)
{
    ...
    VkClearValue clear_values[2]; // One for color, one for depth

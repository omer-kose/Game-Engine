...
b8 load_texture(const char* texture_name, texture* t)
{
    ...
    if(data)
    {
        // We can both load a texture that has not been loaded yet or change the texture of an already loaded one.
        u32 current_generation = t->generation; // save the current generation in case of a valid id
        t->generation = INVALID_ID; // Invalidate after storing the current_generation in any case (just a safe guard)

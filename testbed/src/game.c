...
// TODO: This should not be available outside the engine
...
void recalculate_view_matrix(game_state* state)
{
    ...
    {
        mat4 inv_rotation = mat4_transposed(mat4_euler_xyz(state->camera_euler.x, state->camera_euler.y, state->camera_euler.z));
        mat4 inv_translation = mat4_translation((vec3){-state->camera_position.x, -state->camera_position.y, -state->camera_position.z});

        // R^T * T^-1 but order of multiplication is inverted due to the math library.
        state->view = mat4_mul(inv_translation, inv_rotation);
...
b8 game_initialize(game* game_inst) 
{
    ...
    state->view = mat4_translation((vec3){-state->camera_position.x, -state->camera_position.y, -state->camera_position.z});
    state->camera_view_dirty = true;
...
b8 game_update(game* game_inst, f32 delta_time) 
{
    ...
    // TODO: temp hack to move camera around.
    ...
    // TODO: Should not be available outside the engine
